    ...
    <v-card flat class="border rounded-lg sm-ma-4">
      ...
      <div class="d-flex justify-space-between border">
        ...
        <v-card-title class="text-center ma-2"
          >{{ nomeVeiculo }} - Revisões</v-card-title
        >
      ...
      <div v-if="revisoes.length === 0" class="text-center">
        Nenhuma revisão encontrada para este veículo.
      </div>
...
export default {
  ...
  methods: {
    ...
    async carregarRevisoes() {
      try {
        // Obter o ID do veículo da query da rota
        const idVeiculo = this.$route.query.id;
        if (idVeiculo) {
          // Em seguida, carregue as revisões do veículo
          const responseRevisoes = await axios.get(
            `http://localhost/api/revisoes/?veiculo_id=${idVeiculo}`
          );
          this.revisoes = responseRevisoes.data;

          // Carregue o nome do veículo
          const responseVeiculo = await axios.get(
            `http://localhost/api/veiculos/${idVeiculo}`
          );
          this.nomeVeiculo = responseVeiculo.data.modelo;
